...
def main():
    ...
    lab8.train(images_train, labels_train)
    ...
    plot_9images(images=images_test[samples], cls_idx_true=cls_idx_test[samples],
                 cls_idx_pred=lab8.predict(images_test[samples]), all_cls_names=class_names, smooth=True)

    print(f'\nAccuracy: {(lab8.predict(images_test) == cls_idx_test).mean() * 100}%\n')
